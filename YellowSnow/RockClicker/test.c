...
bool clearData(GameState *gameState) {
  FILE *fl = fopen("data.txt", "w");
  if (!fl) {
    return false;
  }
  gameState->score = 0;
  gameState->clickAmount = 1;
  gameState->cpsLevel = 0;
  gameState->clickInterval = 10000;
  fprintf(fl, "Score:%d\n", gameState->score);
  fprintf(fl, "TotalClicks:%d\n", gameState->clickAmount);
  fprintf(fl, "CPSLevel:%d\n", gameState->cpsLevel);
  fprintf(fl, "ClickInterval:%d", gameState->clickInterval);
...
bool initialize(Resources *res) {
  ...
  if (!res->clickSound) {
    printf("Failed to load click sound effect!Error: %s\n", Mix_GetError());
  }
  res->popSound = Mix_LoadWAV("sounds/pop.mp3");
  if (!res->popSound) {
    printf("Failed to load pops sound effect! Error: %s\n", Mix_GetError());
  }
  ...
  while (SDL_PollEvent(&e)) {
    ...
    case SDL_MOUSEBUTTONDOWN: {
      ...
      if (isInsideRect(&ui->clickBut, mx, my) &&
        ...
        Mix_PlayChannel(-1, res->popSound, 0);
        state->score -= clickUpgradeCost;
        state->clickAmount++;
      }
    ...
    case SDL_KEYDOWN: {
      if (e.key.keysym.sym == SDLK_s) {
        if (saveData(state)) {
          printf("Game manualy saved!\n");
        }
      }
      if (e.key.keysym.sym == SDLK_r) {
        if (clearData(state)) {
          printf("Game data reset!\n");
        }
      }
      break;
    }
...
void render(Resources *res, GameState *state, UIElements *ui) {
  ...
  renderText(res->renderer, res->smallFont, saveInstructions, ui->textColor,
             700, 940);
  char clearInstructions[64] = "press R to reset game";
  renderText(res->renderer, res->smallFont, clearInstructions, ui->textColor,
             700, 970);
